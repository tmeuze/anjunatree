...
for (const { file, size, frac } of targets) {
  const png = encodePng(size, size, render(size, frac))
...
writeFileSync(
  ...
  <defs>
    <!-- userSpaceOnUse, not the objectBoundingBox default: the axis line is
         perfectly horizontal, so its own bounding box has zero height, and a
         vertical gradient's Y coordinates are undefined against that per the
         SVG spec — see the matching comment in src/Brand.tsx. -->
    <linearGradient id="s" gradientUnits="userSpaceOnUse" x1="256" y1="499" x2="256" y2="12">
